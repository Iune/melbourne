...
class ScoreboardFonts:
    ...
    def __post_init__(self):
        self.voter_header = QFont(self.base_font_family, int(14 * self.scaling * self.font_os_scaling))
        self.contest_header = QFont(self.base_font_family, int(14 * self.scaling * self.font_os_scaling))
        self.country = QFont(self.base_font_family, int(12 * self.scaling * self.font_os_scaling))
        self.entry_details = QFont(self.base_font_family, int(12 * self.scaling * self.font_os_scaling))
        self.awarded_pts = QFont(self.pts_font_family, int(14 * self.scaling * self.font_os_scaling))
        self.total_pts = QFont(self.pts_font_family, int(14 * self.scaling * self.font_os_scaling))
...
class ScoreboardColors:
    ...
    black: QColor = _hex_to_rgb("#212121")
    grey_text: QColor = _hex_to_rgb("#C4C4C4")
    white_text: QColor = _hex_to_rgb("#FFFFFF")
    country_text: QColor = _hex_to_rgb("#7E7E7E")
...
class ScoreboardSizes:
    ...
    def __post_init__(self):
        ...
        self.voter_header = QFontMetrics(self.fonts.voter_header).boundingRect(
            f"Now Voting: {contest.voters[self.voter]} ({self.voter + 1}/{contest.num_voters}").width()
        self.contest_header = QFontMetrics(self.fonts.contest_header).boundingRect(
            f"{self.details.title} Results").width()
        ...
        for entry in contest.entries:
            ...
            current_entry = entry_fm.boundingRect(f"{entry.artist} – {entry.song}").width()
        ...
        self.rectangle = max(self.country, self.entry_details) + self.flag_offset + 80 * scaling
        self.width = max(max(30 * scaling + 2 * self.rectangle, 48 * scaling + self.contest_header),
                         10 * scaling + self.voter_header)

        num_entries_in_left_col = int(contest.num_entries / 2) + contest.num_entries % 2
        self.height = 10.0 * scaling + 35 * scaling * num_entries_in_left_col + 70 * scaling


def draw_text(painter, point: QPoint, text: str, font: QFont, color: QColor, flags: Qt.AlignmentFlag):
    painter.setPen(color)
    painter.setFont(font)
    _draw_text_helper(painter, point, text, flags | Qt.AlignVCenter)


def _draw_text_helper(painter: QPainter, point: QPoint, text: str, flags: Qt.AlignmentFlag) -> None:
...
def draw_rectangle(painter: QPainter, point: QPoint, size: QPoint, fill: QColor, border: Optional[QColor] = None,
